...
- `Registry.AdoptExisting` now verifies each candidate directory is an
  ...
  regression — the e2e variant locates the fileblock-node binary via
  `/proc/*/comm` (PID 1 is the host init under `hostPID: true`) and
  SIGTERMs it to reproduce the emptyDir-after-container-restart shape
...
[Unreleased]: https://github.com/middlendian/fileblock-csi/compare/v0.3.8...HEAD
